...
final class PicsumService {
    ...
    let imageStorage: ImageStorageService = .init()
        ...
        dataTask =
            urlSession.dataTask(with: request, completionHandler: { data, response, error in
                ...
                if let jsonData = data {
                    do {
                        let pic: PicsumImage = try JSONDecoder().decode(PicsumImage.self, from: jsonData)
    ...
    func downloadImage(link: String, completion: @escaping ImageCompletion) {
    ...
    func getFavoritedImage(picId: Int) -> UIImage? {
        if let image = imageStorage.retrieveImage(forKey: String(picId)) {
            return image
        }
        return nil
    }
